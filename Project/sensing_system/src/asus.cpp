...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub2 = nh.advertise<std_msgs::Float32>("/humanDistance", 10);
	ros::Publisher pub3 = nh.advertise<std_msgs::Float32>("/carsDistance", 10);
	...
	std_msgs::Float32 human_dis;
	std_msgs::Float32 car_dis;
	...
	while (ros::ok())
	{
		...
		for (int i = 0; i < 640; i++)
		{
			...
			if ((y != 0) && (i >= x + 171) && (i <= y + 171))
			{
				...
				if ((!minHumanSet) && (distance[i] != 0))
				{
					min_human = distance[i];
					minHumanSet = true;
				}

				//Finding closest distance to the humans:
				if ((distance[i] < min_human) && (distance[i] != 0)) { min_human = distance[i]; }
			...
			//First we make sure the detected car is within the sensor reading range
			if ((y2 != 0) && (x2 >= 389) && (x2 <= 1547) && (y2 >= 389) && (y2 <= 1547))
			{
				if ((!minCarSet) && (distance[i] != 0))
				{
					min_cars = distance[i];
					minCarSet = true;
				}
				...
				if ((distance[i] < min_cars) && (distance[i] != 0)) { min_cars = distance[i]; }
				if ((distance[i] <= 2.5) && (distance[i] != 0))
				{
					std::cout << "A car is closer than 2.5 meters" << std::endl;
					//In case a car is detected closer than two and a half meters, the TurtleBot is commanded to stop
					turtle.data = 0;
			...
			//If no obstacles have been detected within close range, the TurtleBot is commanded to go forward
			if (go)
			{
				turtle.data = 1;
				pub.publish(turtle);
